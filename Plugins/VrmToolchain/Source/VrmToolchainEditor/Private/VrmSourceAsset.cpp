...
{
#if VRM_HAS_TAGS_CONTEXT
    // When TagsContext is available, the modern override handles calling Super.
    // For backward compatibility, extract tags via Context approach and add them manually.
#if VRM_HAS_ASSET_REGISTRY_TAG
    TArray<FAssetRegistryTag> Tmp;
    AddVrmTags_Array(Tmp, SourceFilename, VrmSpecVersionMajor, DetectedVrmExtension);
    OutTags.Append(Tmp);
#endif
#else
    // When TagsContext is not available, fall back to calling Super directly.
...
bool UVrmSourceAsset::EditorRecomputeSpecVersionForce()
{
    ...
    {
        UE_LOG(LogVrmToolchainEditor, Warning, TEXT("EditorRecomputeSpecVersionForce: source missing: '%s' (abs='%s')"), *Src, *AbsSrc);
        return false;
    }

    // 2) Get validator exe from module (never re-implement resolver)
    const FVrmToolchainEditorModule& Mod = FModuleManager::LoadModuleChecked<FVrmToolchainEditorModule>("VrmToolchainEditor");
    const FString Exe = Mod.GetValidationToolPath();
    if (Exe.IsEmpty() || !FPaths::FileExists(Exe))
    {
        UE_LOG(LogVrmToolchainEditor, Error, TEXT("EditorRecomputeSpecVersionForce: vrm_validate.exe unavailable. Exe='%s'"), *Exe);
        return false;
    }
    ...
    if (!Proc.IsValid())
    {
        ...
        UE_LOG(LogVrmToolchainEditor, Error, TEXT("EditorRecomputeSpecVersionForce: failed to launch: %s %s"), *Exe, *Args);
        return false;
    }
    ...
    if (StdOut.IsEmpty())
    {
        UE_LOG(LogVrmToolchainEditor, Warning, TEXT("EditorRecomputeSpecVersionForce: empty stdout. rc=%d exe='%s' args='%s'"), ReturnCode, *Exe, *Args);
        return false;
    }
    ...
    TSharedPtr<FJsonObject> RootObj;
    {
        ...
        {
            UE_LOG(LogVrmToolchainEditor, Warning, TEXT("EditorRecomputeSpecVersionForce: JSON parse failed. rc=%d head='%s'"), ReturnCode, *StdOut.Left(200));
            return false;
        }
    }
    ...
    {
        UE_LOG(LogVrmToolchainEditor, Warning, TEXT("EditorRecomputeSpecVersionForce: JSON missing 'files'. head='%s'"), *StdOut.Left(200));
        return false;
    }
    ...
    {
        UE_LOG(LogVrmToolchainEditor, Warning, TEXT("EditorRecomputeSpecVersionForce: JSON missing files[0].vrm"));
        return false;
    }
    ...
    {
        UE_LOG(LogVrmToolchainEditor, Warning, TEXT("EditorRecomputeSpecVersionForce: JSON missing files[0].vrm.version"));
        return false;
    }
    ...
    if (Major < 0)
    {
        UE_LOG(LogVrmToolchainEditor, Warning, TEXT("EditorRecomputeSpecVersionForce: invalid vrm.version '%s'"), *VersionStr);
        return false;
    }
